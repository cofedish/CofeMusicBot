...
class MusicCog(commands.Cog):
    ...
    async def on_command(self, ctx):
        ...
        print(f"Команда {ctx.command} была введена пользователем {ctx.author} в канале {ctx.channel}")

    # Добавляем команды в очередь
    async def add_to_command_queue(self, ctx, command):
        """Добавляем команду в очередь и выводим её в лог."""
        print(f"Добавляем команду {command.__name__} в очередь от {ctx.author}.")
    ...
    async def goyda(self, ctx):
        query = r"C:\Users\COFED\Desktop\Music_bot\CofeMusicBot\cache\Okhlabystin_-_gojjda_76690131.mp3"
        await self.add_to_command_queue(ctx, lambda ctx: self.music_player.add_to_queue(ctx, query))

    @commands.command(name='rickroll', help='Ну нажми че ты')
    async def rick(self, ctx):
        query = r"C:\Users\COFED\Desktop\Music_bot\CofeMusicBot\cache\Rick_Astley_-_Never_Gonna_Give_You_Up_47958276.mp3"
        await self.add_to_command_queue(ctx, lambda ctx: self.music_player.add_to_queue(ctx, query))
